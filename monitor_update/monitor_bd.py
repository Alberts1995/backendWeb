...
class SQLite():
    ...
    def insert_day(self, new, reckl, offer, plan_day, gross, morja, scheme, data):
        sql = """INSERT INTO Day (new, reckl, offer, plan_day, gross, morja, scheme, data) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
        values = (new, reckl, offer, plan_day, gross, morja, scheme, data)
        self.cursor.execute(sql, values)
        

    def insert_month(self, new, reckl, offer, plan_month, gross, morja, scheme, data):
        sql = """INSERT INTO Month (new, reckl, offer, plan_month, gross, morja, scheme, data) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
        values = (new, reckl, offer, plan_month, gross, morja, scheme,data)
        self.cursor.execute(sql, values)
    ...
    def plan_month(self, Month_Plan_FB, Month_Total_Merjinalnost_FB, Month_Total_Gross_FB, Month_Plan_sxema, Month_Total_Merjinalnost_sxema, Month_Total_Gross_sxema, month_Plan_prochee, Month_Total_Merjinalnost_prochee, Month_Total_Gross_prochee):
        sql = """INSERT INTO Month_total (Month_Plan_FB, Month_Total_Merjinalnost_FB, Month_Total_Gross_FB, 
            ...
            Month_Total_Merjinalnost_prochee, Month_Total_Gross_prochee) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
    ...
    def plan_day(self,Day_Plan_FB,  Day_Total_Merjinalnost_FB, Day_Total_Gross_FB, Day_Plan_sxema, Day_Total_Merjinalnost_Sxema, Day_Total_Gross_Sxema, Day_Plan_prochee,  Day_Total_Merjinalnost_prochee, Day_Total_Gross_prochee):
        sql = """INSERT INTO Day_total (Day_Plan_FB,  Day_Total_Merjinalnost_FB, Day_Total_Gross_FB,
        Day_Plan_sxema, Day_Total_Merjinalnost_Sxema, Day_Total_Gross_Sxema, Day_Plan_prochee,  
        Day_Total_Merjinalnost_prochee, Day_Total_Gross_prochee) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        values = (Day_Plan_FB,  Day_Total_Merjinalnost_FB, Day_Total_Gross_FB, Day_Plan_sxema, Day_Total_Merjinalnost_Sxema, Day_Total_Gross_Sxema, Day_Plan_prochee,  Day_Total_Merjinalnost_prochee, Day_Total_Gross_prochee)
        self.cursor.execute(sql, values)
        

    def update_table(self, reckl, gross, morja, table, scheme, data):
        sql = """UPDATE {3} SET gross = '{1:s}', morja = '{2:s}', data = '{5}' WHERE reckl='{0:s}' AND scheme = '{4:s}'
               """.format(reckl,  gross, morja, table, scheme, data)
    ...
    def update_day(self, Day_Plan_FB, Day_Total_Merjinalnost_FB, Day_Total_Gross_FB, Day_Plan_sxema, Day_Total_Merjinalnost_Sxema, Day_Total_Gross_Sxema, Day_Plan_prochee, Day_Total_Merjinalnost_prochee, Day_Total_Gross_prochee):
        sql = """UPDATE Day_total SET Day_Plan_FB = '{0:s}', Day_Total_Merjinalnost_FB = '{1:s}', Day_Total_Gross_FB = '{2:s}', 
        Day_Plan_sxema = '{3:s}', Day_Total_Merjinalnost_Sxema = '{4:s}', Day_Total_Gross_Sxema = '{5:s}', Day_Plan_prochee = '{6:s}', 
        Day_Total_Merjinalnost_prochee = '{7:s}', Day_Total_Gross_prochee = '{8:s}' WHERE id=4
               """.format(str(Day_Plan_FB), str(Day_Total_Merjinalnost_FB), str(Day_Total_Gross_FB), str(Day_Plan_sxema), str(Day_Total_Merjinalnost_Sxema), str(Day_Total_Gross_Sxema), str(Day_Plan_prochee), str(Day_Total_Merjinalnost_prochee), str(Day_Total_Gross_prochee))
        self.cursor.execute(sql)
        
    # def sel_all_day_mont(self):
    #     sql = """ SELECT DISTINCT Day.reckl, Day.offer as total_offer, Day.plan_day, Month.plan_month, Day.morja as day_morja, 
	   #      Day.gross as gross_day, Month.gross as mont_gross, Month.morja as month_morja, Day.new, Day.scheme 
		  #   FROM Day INNER JOIN Month
	   #      ON Day.offer = Month.offer
		  #   WHERE Day.reckl != 0 """
    #     self.cursor.execute(sql)
    ...
    # def sel_all_total_mont_day(self):
    #     sql = """ SELECT * FROM Day_total, Month_total"""
    #     self.cursor.execute(sql)
    #     rows = self.cursor.fetchall()
    #     print([row for row in rows])
    #     self.connection.close()
